const playlists = [
  {
    route: "as-perimenoun-oi-gynaikes",
    spotifyId: "0OVVDFVkx97kfgGr2AJAXr",
    title: "Ας περιμένουν οι γυναίκες",
    subtitle: ["- Και τι μουσική προτιμά ο κος Πάνος;", <br/>, "- Προτιμά κύριο Μελά, κύριο Σφακιανάκη, κύριο Καρρά. Διότι του συνέβη κάτι που επέδρασε στην ψυχή του."],
    featured: true
  },
  {
    route: "sexitude",
    spotifyId: "4NWBEUSqrRnJPYrauu5xFA",
    title: "Sexitude",
    subtitle: "This is not music to make sweet love.",
    featured: true
  },
  {
    route: "hot-breeze",
    spotifyId: "3lN4mXfMY8qZfoU6cZ1RXK",
    title: "Hot breeze",
    subtitle: "Music for an empty beach bar around late noon.",
    featured: true
  },
  {
    route: "music-from-the-basement",
    spotifyId: "5evN3B2AVeLtOPVmEzVV1s",
    title: "Music from the basement",
    subtitle: "Mom banging the door to keep it low.",
    featured: true
  },
  {
    route: "the-whiskey-project",
    spotifyId: "6PHgNcNNUbH70RjJ2xbLzo",
    title: "The Whiskey Project",
    subtitle: "We'll get the music, you'll get the ice.",
    featured: true
  },
  {
    route: "eastern-winds",
    spotifyId: "5s9qGXBeh64UDkeICKHkE7",
    title: "Eastern Winds",
    subtitle: "Sounds from the far east.",
    featured: true
  },
  {
    route: "modern-noir",
    spotifyId: "2WbzoO6tKedAL3pvnhIPoY",
    title: "Modern Noir",
    subtitle: "Takes place in a dark bar of the 00s.",
    featured: true
  },
  {
    route: "join-me-in-my-breakfast-nook",
    spotifyId: "1vfW7IcHgiNJIFpUKj9FU9",
    title: "Join me in my breakfast nook",
    subtitle: "Relaxed, happy music to start a wonderful day.",
    featured: true
  },
  {
    route: "cooking",
    spotifyId: "4DsrrE0tPWNFnuX900yYQd",
    title: "Cooking",
    subtitle: "Don't forget to put some cream fraiche on top.",
    featured: true
  },
  {
    route: "off-radio",
    spotifyId: "1VyqjoYYO0rLcU9QCZHN4m",
    title: "OffRadio.gr",
    subtitle: "Handpicked, 100% organic refreshments from Thessaloniki's favorite online radio station.",
    featured: true
  }
];

export default playlists;
